...
private:
    ...
    struct Event {
      ...
      Event(T d, std::string n, bool b_day = false) :
	    date(d), name(n), related(), is_birthday(b_day), 
            is_child(false), parent(), parent_ptr() {};
    ...
    // we use typedef to define a type alias
    ...
    enum interval_t {daily, weekly, monthly, yearly};  // XXX
    ...
    bool recurring_events(const Date& d, std::string s, int r, 
                          interval_t interval, bool bday=false) {
	...
	bool status = true;
        ...
        //for(int i = 0; i < r; ++i) {  // XXX OFF-BY-ONE OBE ERROR
        for(int i = 1; i < r; ++i) { 
	    switch (interval) {
		case daily:
			++date;
    ...
    static const bool DEBUG = false;  // XXX 
	
    // move_event() -- extra2.1 ================================================

    bool move_event(const Date& from, const Date& to, std::string e, 
                    const Date& parent_date, bool update_parent=false) {
        int diff = to - from;
	...
	for(; it != second; ++it) {
		...
		if(DEBUG) std::cout << "Found event " << event.name << " : " 
                                    << event.date << std::endl;
		...
		if(event.is_child) {
		    Event parent = *event.parent_ptr;
                    ...
                    if(DEBUG) std::cout << event.name << " is a child of " 
	        ...
	        for(; rel_e != event.related.end(); ++rel_e) {
	            ...
	            if(DEBUG) std::cout << "\t** move_event(" << T(rel_e->first) << ", "<< new_date 
                                        << ", " << rel_e->second << ", " << T(to) << ")" << std::endl;
    ...
    // Here is a Functor: class that overload the call operator XXX
    // It allows objects of its type to be used as if they were a function (Lippman p571)
	...
	void operator()(std::pair<T, Event> pair) {
	    ...
	    std::cout << "DTSTART:" << pair.first.year();
            ...
            if(pair.first.month() < 10)  
                std::cout << "0"; std::cout << pair.first.month();

            if(pair.first.day() < 10 ) 
                std::cout << "0";

            std::cout << pair.first.day() << "T080000" << std::endl;
	    std::cout << "DTEND:" << pair.first.year();

            if(pair.first.month() < 10 ) 
                std::cout << "0"; 

            std::cout << pair.first.month();

	    if(pair.first.day() < 10 ) 
                std::cout << "0"; 
         
            std::cout << pair.first.day() << "T090000" << std::endl;
...
public:
    ...
    enum format {list, cal, iCalendar};  //XXX FOR EXTR2.2
    ...
    template<class U> 
    Calendar& operator=(const Calendar<U>& rhs) {
	date = rhs.date;
	events.clear();
    ...
    ***************/
	...
	iterator_t first = events.lower_bound(rel_date), 
                   end = events.upper_bound(rel_date);
	...
	return recurring_events(d, e, r, Calendar<T>::monthly);
        //return recurring_events(d, e, r, Calendar<T>::daily);
    };
