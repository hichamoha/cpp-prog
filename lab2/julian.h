...
different calendars. 2012 is year 6725 of the current Julian Period. 
...
*/
  ...
  public:
    ...
    // Other constructor
    Julian(int yy, int mm, int dd); //  check for valid date and initialize
...
// Åtkomst (9)...For examing the Date (Non modifying operations)
    ...
    virtual int month() const; //första månaden ska representeras av talet 1
    virtual int day() const;   //den första i varje månad ska 
...
// Mutator (7)...For changing the Date (Modifying operations)
    Julian operator++(int); //returnerar kopia av sig själv  
    Julian operator--(int); //returnerar kopia av sig själv   
    virtual Date& operator++(); //returnerar referens till sig själv
    virtual Date& operator--(); //returnerar referens till sig själv
  ...
  private:
    // ****** Help functions: 
    virtual data jd_to_date(double JD) const;
    virtual double date_to_jd(int y,int m ,int d) const;
    bool leap_year(int y) const;
    //virtual bool leap_year(int y) const;  ????????
  
    double julian_to_jd(int y,int m ,int d) const;
    data jd_to_julian(double JD) const;
  };
