...
namespace camelot {
  ...
  // constructor ............................................................
    Actor::Actor() : id(instances) // Constructor initializer list
    {
        ...
        // current_weapon = make_shared<DefaultWeapon>();   
        current_weapon = new DefaultWeapon();   // So everyone can try to fight,
        // Remember to delete this in inherited classes constructors

        // container = make_shared<Container>();
        container = new Bag(); // FIXME Derived-to-base conversion (Lippman 597)!?
        current_room = 0;
  ...
  // go() -----------------------------------------------------------------
    ...
    void Actor::go(std::string exit_name) {
        ...
        if (exit->is_locked()) {
            std::cerr << "\t Exit is locked...Unlock it in order to get through" 
                      << std::endl;
            return;
        }
        ...
        std::cerr << "\t" << get_name() << " entered " << exit_name 
                  << std::endl;
    }
  // fight() ............................................................
  ...
  // loadAct() ----------------------------------------------------------------
                           ...
                           const std::map<std::string, Object *> & objs) 
    {
        ...
        if (type == "Human") {
            ...
            actor = new Player(current_room, hp, strength, 
                               has_heart, max_health);
        } 
        else if (type == "Wizard") {
            bool has_heart = str2int(properties.find("has_heart")->second) != 0;
            int max_health = str2int(properties.find("max_health")->second);
