...
class Matrix
{
 public:
    ...
    private:
       ...
       //class matrix_row can access everything in class Vector
    };
    
    Matrix( );
    Matrix( std::size_t, std::size_t );
    Matrix( const Matrix& );
    Matrix(int size);//XXX should be std::size_t as argument since the other size uses that
    ~Matrix( );
...
std::ostream& operator<< ( std::ostream&, Matrix& ); // XXX should have const Matrix& 
                                                     //otherwise we cannot compile cout << (m*m2) << endl;
