...
Om man anger en storlek till konstruktorn ska elementen initieras till 
defaultvärdet för typen (tilldela värdet T() till elementen). 
...
// constructor with 2 arguments (size, init), init: default value for all elements 
    Vector(const size_t size, T );                                            
...
// Assignment operator
    ...
    const Vector<T>& operator=(const initializer_list<T> ); 
...
//======= Extra functions for test driver ========================
...
// ====== Ny funktionalitet i klassen  Vector ===================
  ...
  private: 
    ...
    int vectorCount;  // # of vectors instantiated
    void extend_vec ();  // to expand the vector 
...
/*
A central problem in programming is the fixed size of arrays. This problem can be
avoided by dynamically allocating vectors. The way to handle this is to make the
vector bigger: Expanding the vector.

Geometric expansion and amortized cost:
To avoid incurring the cost of resizing many times, dynamic arrays resize by a large
amount, such as doubling in size, and use the reserved space for future expansion.
*/

// extend_vec(): resize vector (Ridimensionamento) .............................
// Strategy of DOUBLING of the current capacity
template <typename T>
  void Vector<T>::extend_vec() //(const size_t sz)
{   
  //if (nbElements>capacity/2)
  if (capacity == 0) capacity++;
  //{
     // capacity is the real size of the allocated memory
     capacity *= 2;  // double the previous allocated memory
     ...
     delete[] ptr; // deallocation of the old memory
  //}
     ptr = new_vector;
}  
...
template <typename T>   
  Vector<T>::Vector(const size_t size) : capacity(size)
{
  if (capacity > 0)
  {                    
     nbElements = 0;
     ...
     //size = capacity;   // resize the vector 
...
// init: default value, initialize all elements to a default value  
    
template <typename T>   
  Vector<T>::Vector(const size_t size, T init) : capacity(size)
{
  if (capacity > 0)
  {                    
     nbElements = 0;
...
template <typename T>  
  Vector<T>::Vector(const Vector<T> &copia) : capacity(copia.capacity)  
{
  if (capacity > 0)
  {
    nbElements = 0;
    ...
    extend_vec();     // expand vector if it is necessary 
  }
  else ptr = nullptr; 
}
...
template <typename T>    
  Vector<T>::Vector(const initializer_list<T> &il) : capacity(il.size()) 
{
  if (capacity > 0)
  {
     nbElements = 0;
  ...
  const Vector<T> &Vector<T>::operator=(const Vector<T> &right)
{
  ...
  //RESIZE: for vector of different sizes, deallocate original
  //left side vector then allocate new left side vector
  
  else if (capacity != right.capacity)
  {
     delete [] ptr;  // clear(); // deallocate old memory 
...
//tilldelningsoperator som tar en initializer_list som parameter.
template <typename T>
  const Vector<T> &Vector<T>::operator=(const initializer_list<T> rightlist)
{
  if (ptr != nullptr)
    delete [] ptr;  //clear();

  capacity = rightlist.size();
  nbElements = 0;
  //extend_vec(size);  // Expand the vector to a new size
  //size = capacity;   // resize the vector  
...
default move constructor...BUT IN C++11 ?
...
template <typename T>
  Vector<T>::Vector(Vector<T> &&tomove) noexcept
{
  ptr = tomove.ptr;           //assign the class data members from the source 
  capacity = tomove.capacity; //object to the object that is being constructed
  nbElements = tomove.nbElements;
  
  extend_vec();

  tomove.ptr = nullptr;  //Assign the data members of the source object to 
  tomove.capacity = 0;   //default values. This prevents the destructor from 
                         //freeing resources (such as memory) multiple times.
  //WHAT ABOUT tomove.nbElements ? XXX
} 
  ...
  Vector<T> &Vector<T>::operator=(Vector<T> &&source) noexcept
{
  ...
  capacity = source.capacity; // size from the source object.
  ...
  source.capacity = 0;      // object so that the destructor does not 
  ...
  T & Vector<T>::operator[](const int subscript)
{
   ...
   if ((subscript<0)||(subscript>(int)capacity-1))
...
template <typename U>
  istream &operator>> (istream &input, Vector<U> &v)  // friend function
{
...
template <typename U>
  ostream &operator<< (ostream &output, const Vector<U> &v)  // friend function
{
  unsigned int i;
...
{
  if (nbElements != right.nbElements)
...
//======================================================== sort()
/*
A call to sort() arranges the elements in the input range into sorted order using 
the element type's < operator.
sort() is not stable: equivalent elements that are ordered one way before sorting 
may be ordered differently after sorting.
*/
// sort(bool ascending = true) sorterar vektorn i angiven riktning på
// enklast möjliga sätt. Använd std::sort (datatyper som ska jämföras måste
// definiera operator<.)
  ...
  bool Vector<T>::exists(const T& val)
{
  ...
  return result != ptr[nbElements];  
   ...
   // this function is help method for Matrix class implementations
   // Resize() ................................................ 
   //  New elements are assigned the given value.
   template <typename T>      
        void Vector<T>::resize(size_t new_size, const T& value = T()) {
            ...
            if (new_size > nbElements) {
                // Set the new elements to the default value
                for (size_t i = nbElements; i < new_size; ++i) {
                    ptr[i] = value;  //assign the default value
                }
            }
...
/*
//======================= Auxilliary function to sort(),less_than()  
template <typename T>
  bool Vector<T>::less_than(const T& x, const T& y)
{
    return (x < y);
}
*/
