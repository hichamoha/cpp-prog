...
template <typename T>  
  Vector<T>::Vector(const Vector<T> &copia) : nbElements(copia.nbElements)  
{ 
  capacity = nbElements;
...
template <typename T>    
  Vector<T>::Vector(const initializer_list<T> &il) : nbElements(il.size()) 
{
  capacity = nbElements;
  ...
  if (capacity > 0)
  {
     ...
     extend_vec();  // Expand the vector if it is necessary
     //size = capacity;   // resize the vector
     ptr = new T[capacity];  
     ++vectorCount;
  ...
  const Vector<T> &Vector<T>::operator=(const Vector<T> &right)
{
  ...
  else if (nbElements != right.nbElements)
  {
     delete [] ptr;  // clear();
     ...
     //nbElements = 0;
                             ...
                             //extend_vec();  // Expand the vector to a new size
     //size = capacity;   // resize the vector  
     ptr = new T[capacity];
  ...
  const Vector<T> &Vector<T>::operator=(const initializer_list<T> rightlist)
{
  ...
  capacity = nbElements;
                             //extend_vec();  // Expand the vector to a new size
  ptr = new T[capacity];   // resize the vector  
...
//======================================================== sort()
/*
  call to sort() arranges the elements in the input range into sorted order using 
